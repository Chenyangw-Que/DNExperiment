...
v4的效果不错，这里要在沿用v4设计的同时，再做增广，仿照Restormer，在最后一个decoder 以及skff后，增加一个NAFBlock
...
class MINAFv6(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=1,
                               groups=1,
                               bias=True)
        self.endNAFBlock = NAFBlock(width)
        # UNet的输出处理
...
class MINAFv6Local(Local_Base, MINAFv6):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        MINAFv6.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = MINAFv6(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
